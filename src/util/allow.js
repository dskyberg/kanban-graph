...
/**
 * https://dev.to/bytebodger/better-typescript-with-javascript-4ke5
 */
class Allow {
   throwOnFailure = true;

   aBoolean = (value = false) => {
      if (typeof value !== 'boolean') this.fail(value, 'is not a Boolean');
      return this;
   };

   // eslint-disable-next-line @typescript-eslint/no-empty-function
   aFunction = (value = () => {}) => {
      if (typeof value !== 'function') this.fail(value, 'is not a function');
      return this;
   };
   ...
   anArray = (value = [], minLength = 0, maxLength = Number.MAX_SAFE_INTEGER) => {
      ...
      if (!Array.isArray(value)) this.fail(value, 'is not an array');
   ...
   anArrayOfInstances = (value = [], modelObject = {}, minLength = 0, maxLength = Number.MAX_SAFE_INTEGER) => {
      this.anArray(value)
         .anObject(modelObject)
         .anInteger(minLength, is.not.negative)
         .anInteger(maxLength, is.not.negative);
   ...
   anInstanceOf = (suppliedObject = {}, modelObject = {}) => {
      ...
      modelKeys.forEach((modelKey) => {
         if (!suppliedObject.hasOwnProperty(modelKey)) aKeyIsMissing = true;
         else {
            ...
            else if (isModelValueAnObject) this.anInstanceOf(suppliedValue, modelValue);
         }
      });
      if (aKeyIsMissing) this.fail(suppliedObject, 'is missing a required key');
      return this;
   };

   anInteger = (value = 0, minValue = Number.MIN_SAFE_INTEGER, maxValue = Number.MAX_SAFE_INTEGER) => {
      if (!Number.isInteger(value)) this.fail(value, 'is not an integer');
   ...
   anObject = (value = {}, minNumberOfKeys = 0, maxNumberOfKeys = Number.MAX_SAFE_INTEGER) => {
      ...
      if (typeof value !== 'object' || Array.isArray(value) || value === null) this.fail(value, 'is not an object');
   ...
   aNumber = (value = 0, minValue = Number.MIN_SAFE_INTEGER, maxValue = Number.MAX_SAFE_INTEGER) => {
      if (typeof value !== 'number') this.fail(value, 'is not a number');
      this.checkRange(value, minValue, maxValue);
      return this;
   };

   aReactElement = (value = <></>) => {
      if (!React.isValidElement(value)) this.fail(value, 'is not a React element');
      return this;
   };

   aString = (value = '', minLength = 0, maxLength = Number.MAX_SAFE_INTEGER) => {
      this.anInteger(minLength, is.not.negative).anInteger(maxLength, is.not.negative);
      if (typeof value !== 'string') this.fail(value, 'is not a string');
      this.checkLength(value, minLength, maxLength);
      return this;
   };

   checkLength = (value, minLength = 0, maxLength = Number.MAX_SAFE_INTEGER) => {
      if (value.length < minLength) this.fail(value, 'is too short');
      if (value.length > maxLength) this.fail(value, 'is too long');
   };

   checkRange = (value = 0, minValue = Number.MIN_SAFE_INTEGER, maxValue = Number.MAX_SAFE_INTEGER) => {
      if (value < minValue) this.fail(value, 'is too small');
      if (value > maxValue) this.fail(value, 'is too large');
   };

   fail = (value, message = '') => {
      if (!this.throwOnFailure) return;
      console.error(value);
   ...
   oneOf = (value, allowedValues) => {
      ...
      if (Array.isArray(allowedValues)) {
         if (!allowedValues.some((allowedValue) => value === allowedValue)) this.fail(value, 'is not an allowed value');
         return this;
      }
      const entries = Object.entries(allowedValues);
      if (!entries.some((entry) => entry[1] === value)) this.fail(value, 'is not an allowed value');
      return this;
   };
}
